...
export const onCreateRoom = fireDatabase.ref("/rooms/{roomId}").onCreate(async (_, context) => {
  ...
  console.log(`Room ${roomId} will be deleted at ${new Date(expirationTime)}`);
});

export const deleteExpiredRooms = onSchedule("every 1 minutes", async () => {
  const now = Date.now();
  const expirationRef = database.ref("/roomExpirations");
  const expiredRoomsSnapshot = await expirationRef.orderByChild("expiresAt").endAt(now).once("value");
